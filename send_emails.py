...
def load_sheet_data(
        sheet_name: str, column_name: str) -> List[Tuple[int, str]]:
    ...
    # +2, так как первая строка - заголовок, а enumerate начинается с 0
    return [(i + 2, email) for i, email in enumerate(emails[1:])]

# Функция для отправки писем


def send_emails(email_data: List[Tuple[int, str]], config: Dict,
    ...
    # По умолчанию "Без темы", если поле отсутствует
    subject = config.get('subject', 'Без темы')
...
def main():
    ...
    parser.add_argument(
        ...
        help='Путь к файлу для сохранения отправленных email (по умолчанию: sent_emails.json)')
    parser.add_argument(
        '--invalid-rows',
        type=str,
        default='invalid_rows.json',
        help='Путь к файлу для сохранения номеров строк с неверными адресами (по умолчанию: invalid_rows.json)')
    ...
    # Замените "Emails" на название вашего столбца
    email_data = load_sheet_data("TestTable", "Email")
